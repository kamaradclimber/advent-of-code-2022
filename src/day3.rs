...
impl Rucksack {
    ...
    fn priority(&self) -> u32 {
        ...
        let my_char = self.compartment2().chars().find(|c| set.contains(c)).expect("No duplicate found");
...
pub fn solve(input_file: String, part: u8) {
    ...
    if part == 1 {
        let sum: u32 = lines.map(|line| Rucksack { line: line.to_string() }.priority()).sum();
        println!("Solution is {sum}");
    } else {
        let sum: u32 = lines
            .map(|line| Rucksack { line: line.to_string() })
            .collect::<Vec<Rucksack>>()
            .chunks(3)
            .map(|sacks| {
                let item_frequencies = sacks.iter().flat_map(Rucksack::unique_items).counts();
                let (common_item, _) = item_frequencies.iter().find(|(_char, count)| **count == 3).expect("Elf group does not have a common item");
